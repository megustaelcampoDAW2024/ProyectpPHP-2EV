...
<div class="d-flex">
    <div class="accordion w-75 mr-2" id="accordionExample">
        ...
        <form action="{{ route('cuota.index') }}" method="GET" class="mb-3">
        ...
        @foreach($clientes as $cliente)
            @if($cliente->cuotas->count() > 0)  
                <div class="card">
                    <div class="card-header" id="heading{{ $cliente->id }}"  type="button" data-toggle="collapse" data-target="#collapse{{ $cliente->id }}" aria-expanded="true" aria-controls="collapse{{ $cliente->id }}">
                        <table class="table non-bordered" style="margin-bottom: 0;">
                            <thead>
                                <tr>
                                    <td style="width: 20%; border-top: none;">
                                        <a class="btn btn-primary" href="{{ route('cliente.show', $cliente->id) }}">
                                            Cliente {{ $cliente->id }}
                                        </a>
                                    </td>
                                    <td style="width: 20%; vertical-align: middle; border-top: none;">Nombre: {{ $cliente->nombre }}</td>
                                    <td style="width: 20%; vertical-align: middle; border-top: none;">Teléfono: {{ $cliente->telefono }}</td>
                                    <td style="width: 20%; vertical-align: middle; border-top: none;">CIF: {{ $cliente->cif }}</td>
                                    <td style="width: 20%; vertical-align: middle; border-top: none;">{{ $cliente->correo }}</td>
                                </tr>
                            </thead>
                        </table>
                    </div>
        
                    <div id="collapse{{ $cliente->id }}" class="collapse" aria-labelledby="heading{{ $cliente->id }}" data-parent="#accordionExample">
                        <div class="card-body">
                            <table class="table">
                                <thead>
                                    <tr>
        ...
        <div class="d-flex justify-content-center mt-3">
            {{ $clientes->appends(['remesas_page' => request('remesas_page')])->links('pagination::bootstrap-4') }}
        </div>
    </div>
    <div class="w-25 ml-2">
        <div class="mb-3 d-flex justify-content-between">
        ...
        <div class="d-flex justify-content-center">
            {{ $remesas->appends(['clientes_page' => request('clientes_page')])->links('pagination::bootstrap-4') }}
        </div>
    </div>
</div>
@endsection