    ...
    <tbody>
        @foreach ($users as $user)
            <tr>
                ...
                <td>
                    <a href="{{ route('user.edit', $user->id) }}" class="btn btn-warning w-100">
                        Modificar 
                ...
                @if (Auth::user()->rol == 'A')
                    <td>
                        <a href="#" class="btn btn-danger w-100" data-toggle="modal" data-target="#confirmDeleteModal{{ $user->id }}">
                            Eliminar 
...
@if(Auth::user()->rol == 'A')
    ...
    @endforeach
@elseif(Auth::user()->rol == 'O')
    <div class="modal fade" id="confirmDeleteModal{{ Auth::user()->id }}" tabindex="-1" aria-labelledby="confirmDeleteModalLabel{{ Auth::user()->id }}" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="confirmDeleteModalLabel{{ Auth::user()->id }}">Eliminar usuario</h5>
                    <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
